...
`x` でメニューを開き、`?` ですべてのキーを一覧表示します。メニューから設定パネルと
doctor パネルへ進めます。`s` と `d` でも行けますが、どちらかのパネルが開いているときだけ
です。カード一覧では何もしないので、入口は `x` ひとつです。`Esc` はどこにいても 1 段階
戻ります。パネルならメニューへ、メニューなら閉じます。

設定を順に切り替えると表示中の内容はその場で変わります（`l`/`→` で先へ、`h`/`←` で戻り、
`o`/`↵` も先へ）。`s` を押すまでは何も書き込まれません。
...
`scope = "workspace"` は、ワークスペースごとに 1 つずつ開くことに意味を与える設定です。
デーモンがまだ配置先を特定していないペインは隠さず表示し、scope が `all` にフォールバック
したときはフッター上部にその理由を表示します。
