...
| `stop-daemon` | デーモンを停止 | |
| `update` | 最新リリースを再インストールしてデーモンを再起動。リンクされたチェックアウトは拒否 | [サイドバー](#サイドバー) |
...
あり、かつプラグインが GitHub から入っている場合は `u` で導入し、デーモンを再起動して、
サイドバーを新しいバイナリで再実行します。`update` アクションでは、サイドバー外から再インストールと
デーモン再起動を行えます。どちらもリンクされた作業ディレクトリは拒否し、代わりに `git pull` を
案内します。herdr はリンクを置き換えませんし、そのツリーは編集している人のものです。
