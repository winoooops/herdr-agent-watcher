...
Herdr 0.8.0 以上と Rust 1.88 以上が必要です。Herdr はインストール時に
`cargo build --release` を実行し、ツールチェーンが無い場合はエラーを報告するだけで、
自動インストールはしません。
...
- [ ] 不要になったブリッジディレクトリの回収。判定は**生存性**で — pane id が Herdr の
      ...
      巻き込む）
- [ ] リリースごとにビルド済みバイナリを公開し、インストールに `cargo` を不要にする。
      Herdr にリリースアセットのサポートは文書化されていないが、`[[build]]` は任意の
      コマンドなので、コンパイルの代わりに成果物を取得・検証し、対応するアセットが無い
      プラットフォームでは `cargo build --release` にフォールバックできる。これが入ったら、
      Rust の要件は **インストール** から **ローカル開発** へ移す — ビルドが必要なのは
      プラグインをいじる人だけになる
