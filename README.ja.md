...
呼び出すたびに**意図的に**新しい分割ペインが開きます。バインドするまでキーは割り当てられて
...
キーで開くには **Settings** を開きます。先頭行には設定されたキーと現在の割り当て状態が表示
されます。その行を選んで `o` または `↵` を押すと **Keybindings** が開き、そこで `↵` を押すと
キーを編集し、`b` を押すとすぐに割り当てと解除を切り替えられます。変更は書き込む前に Herdr
自身が検証します。スクリプトからは引き続き同じ操作を次のように実行できます:
...
何がそのキーを使っているかを示します。別のキーにしたいときは、実行する前に
`keys.open_sidebar` を設定してください。

`x` には **Update** もあります（どのパネルからでも `u` で開けます）。GitHub に最新リリースを問い合わせ、実行中のビルドとの関係を
示します。問い合わせるのは開いたときだけで、勝手に通信するものはありません。新しいリリースが
あり、かつプラグインが GitHub から入っている場合は `u` で導入し、そのあとサイドバーを開き直す
よう促します。実行中のバイナリを差し替えられるプロセスは無いからです。リンクされた作業
ディレクトリには代わりに `git pull` を案内します。herdr はリンクへの上書きを拒みますし、その
ツリーは編集している人のものです。

> [!WARNING]
> **Settings** の先頭行から **Keybindings** を開いて割り当てを解除するか、
> `unbind-sidebar-key` を実行してから **プラグインをアンインストールしてください**。Herdr は
> アンインストール時に何も実行しないため、そのままでは割り当てが指し示す先のアクションより
> 長く残ります。
...
同じレポートはサイドバーからすぐ見られます。`x` を押して doctor の行を選ぶだけで、`r` で
再生成できます。Herdr のペイン外で見たいときやスクリプトから使いたいときは、コマンドラインを
使ってください。
...
- [x] サイドバー内の設定パネルと doctor パネル。よく使う設定とよく使う診断の両方が、
      ペインを離れずに済むようになった
- [x] 設定パネル自身が指示するデーモン再起動を乗り切る。カードを画面に残し、経過秒数を数え、
      キー入力を待つ行き止まりで終わらずに再購読する
- [x] サイドバーに「古い」と伝える。プラグインを更新しても、開いたままのサイドバーは古い
      バイナリで動き続け、画面には何の表示もない — 0.1.4 のテスト中に実際に惑わされた。
      hello がデーモンのビルドを運び、自分のものと違えばサイドバーが通知を固定表示する
- [x] メニューの**確認 / 更新**の行。押したときだけ GitHub に問い合わせ、描画ループの外で
