...
**Claude のカードで CONTEXT、CACHE、COST が `—` になる。** この 3 つはステータスライン経由
でしか届かないため、ブリッジが有効である必要があります。有効かどうかは
[`doctor`](#doctor) が教えてくれます。有効なら、そのペインは有効化以降まだステータスラインを
描画していない（プロンプトを送ってください）か、サブエージェントを実行中です。後者では
ステータスラインがサブエージェントを表すため、カードにはそのモデル名が出て使用量はゼロから
始まります。メインセッションの数値は次のターンで戻ります。
...
のものであり、Herdr のものではありません。Herdr は認識できないテーブルを無視するので、
誤ったファイルに書いても `herdr config check` 以外では何も知らせません。正しいディレクトリは
`herdr plugin list` が表示します。
